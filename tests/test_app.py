...
@pytest.fixture
def app_instance():
    """Provides a ShortcutApp instance for tests."""
...
def test_universal_mcp_shortcut_app_initialization(app_instance):
    """
    Test that the ShortcutApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "shortcut", "ShortcutApp instance has unexpected name."


def test_universal_mcp_shortcut_tool_docstrings_format(app_instance):
    """
    Test that each tool method in ShortcutApp has a well-formatted docstring,
...
def test_universal_mcp_shortcut_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in ShortcutApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
